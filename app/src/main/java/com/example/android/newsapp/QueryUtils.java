...
public class QueryUtils {
    //Private constructor - no one should ever create an object of QueryUtils.
    private QueryUtils() {
    }
    ...
    private static List<Story> extractFeatureFromJson(String storyJSON) {
        ...
        try {
            ...
            // Extract the "results" array, which contains news results (stories)
            JSONArray resultsArray = responseObject.getJSONArray("results");

            // For each story in the resultsArray, create a Story object
            for (int i = 0; i < resultsArray.length(); i++) {

                // Get a single story at position "i" within the list of news
                JSONObject currentStory = resultsArray.getJSONObject(i);

                // For that story, extract "webTitle", "sectionName" and "webUrl"
                ...
                // Create Book java object with the extracted title
    ...
    public static List<Story> fetchNewsData(String requestUrl) {
        ...
        List<Story> news = extractFeatureFromJson(jsonResponse);
